...
El sistema opera utilizando un patrón arquitectónico fuertemente acoplado a **Next.js 14/15 App Router** y al paradigma de **Server Actions**. Toda la interacción desde el frontend ocurre a través de llamadas RPC seguras de extremo a extremo (`src/app/actions/`), sin pasar por endpoints REST tradicionales.

La única excepción es una **API REST mínima para integraciones externas** (`src/app/api/v1/`), pensada para consumo máquina-a-máquina fuera del frontend:
- `GET /api/v1/health`: healthcheck público, verifica la conexión a la base de datos.
- `GET/POST /api/v1/clientes`: acceso al directorio de clientes protegido por API key (`API_SECRET_KEY`), no por sesión de usuario. Esta variable es **obligatoria**: la ruta deniega todas las solicitudes si no está configurada.
...
# API v1 (integraciones externas) — obligatoria, sin ella /api/v1/clientes deniega todo
API_SECRET_KEY="genera-un-secreto-largo-y-aleatorio"
```
